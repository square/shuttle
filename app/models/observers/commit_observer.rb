...
class CommitObserver < ActiveRecord::Observer
  ...
  def ping_github_webhook(commit)
    if commit.project.repository_url && commit.project.github_webhook_url && commit.previous_changes.include?(:ready) && commit.ready? # if it just became ready
      GithubWebhookPinger.perform_once(commit.id)
    end
  end

  def ping_stash_webhook(commit)
    if commit.project.repository_url && commit.project.stash_webhook_url && [:id, :ready, :loading].any? { |field| commit.previous_changes.include?(field) }
