...
# This is an observer on the {KeyGroup} model.
# 1) Triggers imports and re-imports of KeyGroups, when necessary.
...
class KeyGroupObserver < ActiveRecord::Observer
  ...
  def after_commit(key_group)
    key_group.import! if KeyGroup::FIELDS_THAT_REQUIRE_IMPORT_WHEN_CHANGED.any? {|field| key_group.previous_changes.include?(field)}
  end
end
