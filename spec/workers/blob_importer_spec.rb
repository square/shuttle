  ...
  describe "#perform" do
    context "[rescue Git::BlobNotFoundError]" do
      it "adds import errors to commit in redis when blob importer fails due to a Git::BlobNotFoundError" do
        ...
        expect(@commit.reload.import_errors).to eql([["Git::BlobNotFoundError", "Blob not found in git repo: abc123 (failed in BlobImporter for commit_id #{@commit.id} and blob abc123)"]])
      end
    end
  end
end
