  ...
  describe "#perform" do
    context "[rescue Git::CommitNotFoundError]" do
      it "adds import errors to commit in redis when key creator fails due to a Git::CommitNotFoundError" do
        ...
        expect(commit.import_errors_in_redis).to eql([["Git::CommitNotFoundError", "Commit not found in git repo: abc123 (it may have been rebased away) (failed in KeyCreator for commit_id #{commit.id} and blob abc123)"]])
      end
    end
  end
end
