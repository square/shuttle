...
describe CommitCreator do
  describe "#perform" do
    ...
    context "[rescue Project::BlankRepositoryUrlError]" do
      before :each do
      ...
      it "rescues from Project::BlankRepositoryUrlError error and sends an email" do
        CommitCreator.new.perform(@project.id, "xyz123", {"other_fields" => {:user_id => @user.id}})
        expect(ActionMailer::Base.deliveries.size).to eql(1)
        mail = ActionMailer::Base.deliveries.first
        expect(mail.to).to eql(["example@squareup.com"])
        expect(mail.subject).to eql("[Shuttle] Import Failed for sha: xyz123")
        expect(mail.body).to include("Error Class:   Project::BlankRepositoryUrlError", "Error Message: repository_url is empty")
      end

      it "rescues from Project::BlankRepositoryUrlError error but doesn't send an email if given user_id is nil" do
        CommitCreator.new.perform(@project.id, "xyz123", {:other_fields => {:user_id => nil}})
        expect(ActionMailer::Base.deliveries.size).to eql(0)
      end
    end
