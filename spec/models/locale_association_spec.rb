...
RSpec.describe LocaleAssociation do
  context "[CRUD]" do
    it "requires source_rfc5646_locale, target_rfc5646_locale, checked, uncheck_disabled fields to be present" do
      ...
      expect(association.errors.full_messages).to include("Checked not acceptable")
      expect(association.errors.full_messages).to include("Uncheck disabled not acceptable")
      expect(association.errors.full_messages).to include("Source rfc5646 locale can’t be blank")
      expect(association.errors.full_messages).to include("Target rfc5646 locale can’t be blank")
    end

    it "doesn't allow target and source locales to be equal" do
      association = FactoryBot.build(:locale_association, source_rfc5646_locale: 'fr', target_rfc5646_locale: 'fr')
      association.save
    ...
    it "doesn't allow setting uncheck_disabled if checked is not set" do
      association = FactoryBot.build(:locale_association, checked: false, uncheck_disabled: true)
      association.save
    ...
    it "doesn't allow duplicate source->target pairs" do
      ...
      la = FactoryBot.build(:locale_association, source_rfc5646_locale: 'fr', target_rfc5646_locale: 'fr-CA')
      la.save
    ...
    it "doesn't allow an association between locales from different families (fr -> es-US)" do
      association = FactoryBot.build(:locale_association, source_rfc5646_locale: 'fr', target_rfc5646_locale: 'es-US')
      association.save
